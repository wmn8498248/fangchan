		...
		<view class="service-list" v-if="lists.length>0">
			<navigator class="item  mb10" v-for="(item,index) in lists" :key="index" hover-class="none"
				...
				<view class="info flex1">
					...
					<view class="gray f12 mtb5">{{item.area}}㎡/{{item.buju}}/{{item.chaoxiang}}</view>
					<view class="tags">
						<view class="tag" v-for="son in teseArr(item.liangdian)" :key="son">{{son}}</view>			
					</view>
...
<script>
	import DsZufang from '../../components/ds/ds-zufang.vue'
	...
	export default {
		...
		methods: {
			...
			async getLists() {
				...
				let res = await this.$u.post('/api/listlease',this.form)
