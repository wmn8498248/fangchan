	...
	<view class="container">
		...
		<view class="service-list" v-if="lists.length>0">
			<navigator class="item  mb10" v-for="(item,index) in lists" :key="index" hover-class="none"
				:url="'../news/show?id='+item.estateid">
				<image :src="item.mainpic" mode="scaleToFill" class="thumb mr10 radius10">
				</image>
				<view class="info flex1">
					<view class="f16 bold flex between align-center">{{item.estatename}} <view class="status">{{item.sealstatus}}</view></view>
					<view class="gray f12 mtb5">{{item.areaname}}</view>
					<view class="tags">
						<view class="tag" v-for="son in teseArr(item.biaoqian)" :key="son">{{son}}</view>			
					</view>
					<view class="flex align-center mt5 between">
						<text class="f18 bold red">{{item.minPrice}}万/㎡</text>
						<text class="f12 gray">建面{{item.minMianji}}-{{item.maxMianji}}㎡</text>
					</view>
				</view>
			</navigator>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getLists() {		 
				let res = await this.$u.post('/api/listlp',this.form)
