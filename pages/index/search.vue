<template>
	<view class="container">
		<uni-search-bar @confirm="search" v-model="keywords" @cancel="cancel"></uni-search-bar>
		<view class="tab">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getLists() {
				let res = await this.$u.post('/api/homesearch', {
					title: this.keywords,
					pageNum: this.pageNum,
					pageSize: 10
				})
				console.log(res.data)
				this.newhouseinfo = res.data.newhouseinfo.list
				this.oldhouseinfo = res.data.oldhouseinfo.list
				this.total_new = res.data.newhouseinfo.count
				this.total_old = res.data.oldhouseinfo.count
